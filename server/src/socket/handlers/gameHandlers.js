  ...
  return {
    [CLIENT_EVENTS.START_GAME]: () => {
      try {
        ...
        io.to(roomId).emit(SERVER_EVENTS.GAME_STARTED, game);
        io.to(roomId).emit(SERVER_EVENTS.GAME_UPDATE, game);
    ...
    [CLIENT_EVENTS.SEND_CLUE]: (data) => {
      try {
        ...
        // Broadcast update
        io.to(roomId).emit(SERVER_EVENTS.CLUE_RECEIVED, {
          clue: game.clue,
          team
        });
        io.to(roomId).emit(SERVER_EVENTS.GAME_UPDATE, game);
    ...
    [CLIENT_EVENTS.SELECT_WORD]: (data) => {
      try {
        ...
        // Broadcast update
        io.to(roomId).emit(SERVER_EVENTS.WORD_SELECTED, {
          cardId,
          team
        });
        io.to(roomId).emit(SERVER_EVENTS.GAME_UPDATE, game);
    ...
    [CLIENT_EVENTS.END_TURN]: (data) => {
      try {
        ...
        // Broadcast update
        io.to(roomId).emit(SERVER_EVENTS.TURN_ENDED, {
          previousTeam: team,
          nextTeam: game.turn
        });
        io.to(roomId).emit(SERVER_EVENTS.GAME_UPDATE, game);
